<template>
    <div>
        <div class="flex items-center gap-2">
            ...
            <n-input-group>
                <n-input v-model:value="filterPassport" size="small" placeholder="Поиск по паспорту" @keydown.enter.prevent="loadCredits" />
        ...
        <NModal v-if="selectedCredit" v-model:show="creditDestructureDialog" size="small">
            <CreditRestructureForm
                ...
                client-type="fiz"
                style="width: 50%;"
                @submit="restructureFizClientCredit"
            />
        </NModal>
    </div>
</template>
...
import { creditCreditList, creditCreditDestroy, creditCreateCreditPayoffCreate, creditRestructureCreditCreate, creditPledgeCreate,
    type CreditRestructureCreateRequest, type Credit, type PledgeRequest } from '@/api';
import {
...
function loadCredits() {
    ...
    creditCreditList({
        query: {
            fiz_clients_only: true,
            fiz_client__passport: filterPassport.value,
...
function restructureFizClientCredit(credit_data: CreditRestructureCreateRequest, pledge?: PledgeRequest) {
    ...
    if (selectedCredit.value) {
        creditRestructureCreditCreate({
            ...
            .then(async (response) => {
                ...
                const newCreditPassport = response.data?.passport;
                message.info(`Был добавлен кредит с номером счета ${newCreditUuid} физическому лицу (паспорт ${newCreditPassport}). Старый кредит с номером счета ${oldUuid} был удалён!`)
                if (pledge) {
